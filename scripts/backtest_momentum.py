#!/usr/bin/env python3
"""
Backtesting de la estrategia Momentum.

Descarga datos historicos de BTC, ETH, SOL, BNB y XRP desde Binance
y simula la estrategia de momentum (compra en subidas fuertes con
tendencia alcista confirmada).

Uso:
    python3 scripts/backtest_momentum.py [--days 365] [--budget 30]
"""
...
ASSET_CONFIGS: dict[str, dict[str, float | int]] = {
    "BTCUSDT": {
        "momentum_threshold": 0.05,   # Compra si sube >5% en 24h
        "take_profit":        0.10,   # Vende al +10%
        "stop_loss":         -0.05,   # Corta al -5%
        "trend_days":         7,      # Confirmar tendencia en 7 dias
    },
    "ETHUSDT": {
...
MOMENTUM_ASSETS = list(ASSET_CONFIGS.keys())
MIN_ORDER_USDT = 10.0
...
def is_uptrend(closes: list[float], trend_days: int) -> bool:
    """Verifica si los precios de cierre muestran tendencia alcista.

    Criterio: precio final > precio inicial en la ventana,
    y al menos la mitad de los dias cerraron al alza.
    """
    if len(closes) < 2:
        return False
    ...
    # Al menos la mitad de los dias cerraron al alza
...
def run_backtest(
    ...
    budget: float,
    asset_configs: dict[str, dict[str, float | int]] | None = None,
) -> BacktestResult:
    """Simula la estrategia Momentum con params por moneda."""
    ...
    for i, date in enumerate(sorted_dates):
        ...
        # 2. Check momentum buys
        # Construir historial de cierres para cada moneda (ventana de trend_days)
        ...
        for symbol in symbols:
            ...
            if change is None:
                continue
            cfg = cfgs[symbol]
            threshold = float(cfg["momentum_threshold"])
            trend_d = int(cfg["trend_days"])

            # Solo comprar si sube mas del umbral
            if change < threshold:
                continue

            # Verificar tendencia alcista en los ultimos N dias
            df = prices.get(symbol)
            if df is None:
                continue
            # Obtener cierres hasta la fecha actual
            mask = df.index <= date
            recent_closes = df.loc[mask, "close"].tolist()
            if not is_uptrend(recent_closes, trend_d):
                continue

            # No comprar si ya tenemos posicion
            already_in = any(p.symbol == symbol for p in positions)
            if not already_in:
                momentum_assets.append((symbol, change))

        if momentum_assets and cash >= MIN_ORDER_USDT:
            per_buy = cash / len(momentum_assets)
            for symbol, change in momentum_assets:
                ...
                cfg = cfgs[symbol]
                tp = round(
                    price * (1 + float(cfg["take_profit"])), 2,
                )
                sl = round(
                    price * (1 + float(cfg["stop_loss"])), 2,
                )
                result.trades_log.append({
                    "date": str(date.date()),
                    "action": "BUY",
...
def print_report(result: BacktestResult, days: int) -> str:
    """Genera reporte legible del backtesting momentum."""
    pnl_pct = (
    ...
    lines = [
        "",
        "=" * 65,
        "  BACKTESTING MOMENTUM - RESULTADOS",
        "=" * 65,
        "",
    ...
    for symbol, cfg in ASSET_CONFIGS.items():
        ...
        thr = float(cfg["momentum_threshold"]) * 100
        tp = float(cfg["take_profit"]) * 100
        sl_val = float(cfg["stop_loss"]) * 100
        td = int(cfg["trend_days"])
        lines.append(
            f"    {nm:4s} | Umbral: +{thr:.0f}%"
            f" | TP: +{tp:.0f}%"
            f" | SL: {sl_val:.0f}%"
            f" | Trend: {td}d"
        )
        ...
        for t in result.trades_log:
            ...
            if t["action"] == "BUY":
                ...
                lines.append(
                    ...
                    f" ${t['price']:>10,.2f}"
                    f" | {ps}${t['profit']:.2f}"
                    f" ({_fmt_sign(t['pnl_pct'])}{t['pnl_pct']:.1f}%)"
                    f" | {t['hold_days']}d"
                )
        lines.append("")
...
def main() -> None:
    ...
    print("\nEjecutando backtesting momentum...")
    result = run_backtest(prices, args.budget)
    ...
    rpath = out_dir / "backtest_momentum_report.txt"
